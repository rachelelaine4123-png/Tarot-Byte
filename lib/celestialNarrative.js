...
// This is where the "Major Arcana = fate / Suits = choices" voice lives, and
// where non-decan cards read as AMPLIFIED rather than precise.
//
// NAMING:
//   T = Classic (card + meaning)
//   Z = Astral Threads (zodiac sign + ruling planet overlay)
//   D = The Decan Engine (exact 10° decan degree)
// Major Arcana (governing forces / fate)
...
export function amplifiedText(card, cel) {
  switch (cel.mode) {
    ...
    case "fate": // Majors
      if (cel.kind === "element") {
        return `${card.name} is a Major Arcana card of the element ${cel.body} — a governing force, not a fine degree. The Majors speak the language of fate: something larger than daily choice is moving through the reading. This is about what's being asked of you, not what you're choosing. Meet it, don't manage it.`;
      }
      if (cel.kind === "planet") {
        return `${card.name} is a Major Arcana card ruled by ${cel.body} — an archetypal, planet-sized force acting on you. The Majors are the headlines of a reading: destiny, timing, the hand you're dealt. The energy of ${cel.body} is the current you're inside of. Align with it rather than resist.`;
      }
      const signQ = SIGN_QUALITIES[cel.body] || cel.body;
      return `${card.name} is a Major Arcana card carrying the whole sign of ${cel.body} — a season of life, not a single day. ${cel.body} is ${signQ}. The Majors read as fate and divination: a big current you're inside of. The universe is moving here; your work is to recognize the season and move with it.`;
    case "persona": // Courts
...
function amplifiedNote(cel, card) {
  switch (cel.mode) {
    ...
    case "fate":
      return `No single decan — ${card.name} is a Major Arcana card, fate-sized. At the Decan Engine layer it reads as a governing headline: the universe is moving here, not just your choices. The decan can't narrow what a Major Arcana card already commands.`;
    case "persona":
...
export function readingTone(cardsWithCelestial) {
  ...
  if (counts.fate >= Math.ceil(total / 2)) {
    return { key: "fate", text: "The Major Arcana dominates — this reading is about fate, not fine control. The universe is moving; your work is to align with what's being asked of you." };
  }
