...
import { RADIANT_ORACLE } from "./oracleDeck.js";
...
function drawOracle(count = 1) {
  return shuffle(RADIANT_ORACLE).slice(0, count);
}
...
export const SPREADS = {
  ...
  "energy-reading": {
    ...
    usesOracle: true, // clarified with a Radiant Oracle sign card
    contexts: ["Love", "Career", "Fortune", "General"],
    description:
      "TarotByte's signature spread. Three cards map you, your current energy, and the outcome or shift required — then a Radiant Oracle card clarifies the celestial energy at play.",
  },
};
...
function templatedText(spread, cards, { context, oracle } = {}) {
  ...
  spread.positions.forEach((pos, i) => {
    ...
    const ctx = context ? ` In the context of ${context.toLowerCase()}, ` : " ";
    lines.push({
      position: pos.label,
      card: `${card.name}${orientation}`,
      text: `${pos.label}: ${card.name}${orientation}.${ctx}${meaningFor(card)}`,
    });
  });
  if (spread.usesOracle && oracle) {
    lines.push({
      position: "Radiant Oracle Clarifier",
      card: `${oracle.sign} — ${oracle.keyword}`,
      text: `The Radiant Oracle draws ${oracle.sign} (${oracle.keyword}). ${oracle.energy} ${oracle.clarifier}`,
    });
  }
  return lines;
}
...
export function buildAIPrompt(reading) {
  ...
  const oracleLine = reading.oracle
    ? ` Radiant Oracle clarifier: ${reading.oracle.sign} (${reading.oracle.keyword}).`
    : "";
