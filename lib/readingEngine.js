...
// `uprightPositions` is a Set of position INDEXES that must never come out
// reversed. Used for directional positions (e.g. the Energy Shift) where a
// reversed card would invert the guidance and stop making sense: that slot is
// telling you which way to move, so it is always read upright.
function drawCards(count, { allowReversed = true, deck = TAROT_DECK, uprightPositions = null } = {}) {
  const shuffled = shuffle(deck);
  return shuffled.slice(0, count).map((card, i) => {
    const forcedUpright = uprightPositions?.has(i);
    return {
      ...card,
      reversed: allowReversed && !forcedUpright ? Math.random() < 0.5 : false,
      alwaysUpright: Boolean(forcedUpright),
...
export const SPREADS = {
  ...
  "energy-reading": {
    ...
    positions: [
      ...
      // alwaysUpright: this position is DIRECTIONAL — it names the shift to
      // move toward. A reversed card here would invert the instruction and
      // muddy the guidance, so it is always drawn and read upright.
      { key: "shift", label: "Outcome / Energy Shift Required", alwaysUpright: true },
    ],
...
export function generateReading({ spreadId, context = null, tier = "T" }) {
  ...
  // Positions flagged `alwaysUpright` in the spread definition never draw
  // reversed (e.g. the Energy Shift, which must stay directional).
  const uprightPositions = new Set(
    spread.positions.reduce((acc, p, i) => (p.alwaysUpright ? [...acc, i] : acc), [])
  );

  const cards = drawCards(spread.cards, { uprightPositions });
